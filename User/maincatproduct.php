...
if (isset($_GET['id'])) {
    $id = $_GET['id'];  
    $aproquery = "select * from `product` p join `sub_category` sc on p.pro_subcat=sc.subcat_id join `category` c on sc.category=c.cat_id join `main_category` mc on c.main_category=mc.maincat_id join `pro_badge` pb on p.pro_badge=pb.badge_id join `pro_warrenty` pw on p.pro_warranty=pw.warren_id where p.pro_status = 'Active' AND mc.maincat_id = '$id'";
    $aproresult = mysqli_query($conn, $aproquery);
    $count = mysqli_num_rows($aproresult);

    $catnamequery = "select * from main_category where maincat_id = '$id'";
...
<div class="page-header">
                <div class="container">
                    <h1 class="page-title mb-0"><?php echo $catnamefetch['maincat_name'] ?></h1>
                </div>
            </div>
            <!-- End of Page Header -->
<!-- Start of Main -->
            ...
            <nav class="breadcrumb-nav">
                <div class="container">
                    <ul class="breadcrumb bb-no">
                        ...
                        <li><?php echo $catnamefetch['maincat_name'] ?></li>
                    </ul>
                </div>
            </nav>
            <!-- End of Breadcrumb -->
